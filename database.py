...
class Database:
    ...
    def _init_db_(self):
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS accounting (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, type TEXT NOT NULL, color TEXT NOT NULL, stock INTEGER NOT NULL, date TEXT NOT NULL, image_path TEXT)''')
        self.base.commit()
    ...
    def _insert_(self, data):
        cursor = self.base.cursor()
        cursor.execute('''INSERT INTO accounting (name, type, color, stock, date) VALUES (?, ?, ?, ?, ?)''', (data['name'], data['type'], data['color'], int(data['stock']), data['date']))
        self.base.commit()

    def _update_(self, data):
        cursor = self.base.cursor()
        cursor.execute('''UPDATE accounting SET name=?, type=?, color=?, stock=?, date=? WHERE id=?''', (data['name'], data['type'], data['color'], int(data['stock']), data['date'], data['id']))
        self.base.commit()